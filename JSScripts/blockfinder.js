...
function initMap() {
  ...
  function search_click() {
    ...
    $.post("phpScripts/blockFinder.php", {
      lat: latitude,
      long: longitude,
      nhoodid: "1"
    }, function(data) {
  ...
  function submitclk() {
    ...
    for (var i = 0; i < formatted_adr.length; i++) {
      ...
      switch (formatted_adr[i].types[0]) {
        case "street_number":
          street_number = formatted_adr[i].long_name;
          break;
        case "route":
          address1 = formatted_adr[i].long_name;
          break;
        case "neighborhood":
          address2 = formatted_adr[i].long_name;
          break;
        case "administrative_area_level_1":
          state = formatted_adr[i].long_name;
          break;
        case "locality":
          city = formatted_adr[i].long_name;
          break;
        case "postal_code":
          zip = parseInt(formatted_adr[i].long_name);
          break;
    ...
    // var bid1;
    var blockid = $('#blocks_dropdown').find('option:selected').val();
    $.post("phpScripts/blockFinder.php", {
      join: "1",
      streetadr1: street_number + " " + address1,
      streetadr2: address2,
      state: state,
      zip: zip,
      city: city,
  ...
  function drawpolygon(obj) {
    ...
    var triangleCoords = [{
        lat: parseFloat(obj.nw_lat),
        lng: parseFloat(obj.nw_long)
      }, {
        lat: parseFloat(obj.ne_lat),
        lng: parseFloat(obj.ne_long)
      }, {
        lat: parseFloat(obj.se_lat),
        lng: parseFloat(obj.se_long)
      }, {
        lat: parseFloat(obj.sw_lat),
        lng: parseFloat(obj.sw_long)
      }
